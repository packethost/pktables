			...
			if ip['address_family']==4:
				...
				for k,network in networks.iteritems():
					if IPAddress(ip['address']) in network:
						...
						break
				if not match:
						orphans[ip['address']] = 'iptables -A %s -s %s -p tcp -j ACCEPT -m comment --comment "%s %s v%i"' % (CHAIN, ip['address'], d.hostname, public, ip['address_family'])
	if args.dryrun:
		print "<< dry run >>"
		print("iptables -F %s" %CHAIN)
		for k,network in sorted(networks.iteritems()):
			print("iptables -A %s -s %s -p tcp -j ACCEPT" % (CHAIN, network.cidr))
		...
		for k,network in sorted(networks.iteritems()):
			target.write("iptables -A %s -s %s -p tcp -j ACCEPT\n" % (CHAIN, network.cidr))
		for k,v in sorted(orphans.iteritems()):
			target.write(v)
			target.write("\n")
